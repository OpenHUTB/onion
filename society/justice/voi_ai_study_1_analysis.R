...
# Read study data.
raw_study_data <- read.csv('voi_ai_study_1_data.csv')
...
# Extract marginalized predictions.
principle_choice_prediction_df <- ggeffect(
  principle_choice.glm,
  terms = 'Condition',
  interval = 'confidence') %>%
  dplyr::rename(
    Condition = x,
    Chose_Prioritarian = predicted,
    Std_Err = std.error,
...
# Visualize.
ggplot(data = principle_choice_prediction_df) +
  aes(x = Condition, y = Chose_Prioritarian) +
  geom_linerange(aes(ymin = CI_95_Lower, ymax = CI_95_Upper)) +
  geom_point(size = 2) +
  geom_line(aes(group = Group),
            linetype = 'dashed') +
  geom_jitter(data = study_df,
              height = 0.025,
              width = 0.05,
              size = 0.25,
              color = 'gray') +
  scale_x_discrete(labels = c('Control', 'Veil of Ignorance')) +
  labs(x = 'Condition', y = 'Chose prioritarian') +
  theme_bw()
...
# Extract marginalized predictions.
repeat_choice_prediction_df <- ggeffect(
  repeat_choice.glm,
  terms = c('Condition', 'Motiv_To_Change'),
  interval = 'confidence') %>%
  dplyr::rename(
    Condition = x,
    Repeat = predicted,
    Motiv_To_Change = group,
    Std_Err = std.error,
    CI_95_Lower = conf.low,
    CI_95_Upper = conf.high)

# Visualize.
ggplot(data = repeat_choice_prediction_df) +
  aes(x = Condition, y = Repeat, fill = Motiv_To_Change) +
  ...
  scale_fill_discrete(labels = c('No motivation to change',
                                 'Motivation to change')) +
  labs(x = 'Condition',
       y = 'Repeat principle choice') +
  theme_bw() +
  theme(legend.title = element_blank())

# Summarize proportions of participants repeating with motivation to maximize.
motiv_to_maximize_df <- study_df %>%
  filter(Motiv_To_Maximize == T) %>%
  group_by(Condition) %>%
  dplyr::summarize(Repeat = mean(Repeat))
motiv_to_maximize_df

# Summarize proportions of participants repeating with motivation to prioritize.
motiv_to_prioritize_df <- study_df %>%
  filter(Motiv_To_Prioritize == T) %>%
  group_by(Condition) %>%
  dplyr::summarize(Repeat = mean(Repeat))
motiv_to_prioritize_df

# Inspect cell counts. Insufficient for Breslow-Day test.
freq_tab_df <- study_df %>%
  filter(Motiv_To_Change == T) %>%
  group_by(Condition, Repeat) %>%
  dplyr::summarize(Motiv_To_Maximize = sum(Motiv_To_Maximize),
                   Motiv_To_Prioritize = n() - Motiv_To_Maximize,
                   .groups = 'keep') %>%
  mutate(Repeat = dplyr::recode(Repeat, `0` = 'Change', `1` = 'Repeat'))
xtabs(
  cbind(Motiv_To_Maximize, Motiv_To_Prioritize) ~ Condition + Repeat,
  freq_tab_df)
...
# Extract marginalized predictions.
fairness_reasoning_prediction_df <- ggeffect(
  fairness_reasoning.glm,
  terms = 'Condition',
  interval = 'confidence') %>%
  dplyr::rename(
    Condition = x,
    Fairness_Reasoning = predicted,
    Motiv_To_Change = group,
    Std_Err = std.error,
    CI_95_Lower = conf.low,
    CI_95_Upper = conf.high)
...
# Visualize.
ggplot(data = fairness_reasoning_prediction_df) +
  aes(x = Condition, y = Fairness_Reasoning) +
  geom_linerange(aes(ymin = CI_95_Lower, ymax = CI_95_Upper)) +
  geom_point(size = 2) +
  geom_line(aes(group = Motiv_To_Change),
            linetype = 'dashed') +
  geom_jitter(data = study_df,
              height = 0.025,
              width = 0.05,
              ...
              color = 'gray') +
  scale_x_discrete(labels = c('Control', 'Veil of Ignorance')) +
  labs(x = 'Condition', y = 'Fairness-based reasoning') +
  theme_bw()

# Branch dataframe to estimate effect of fairness-based reasoning on reflective
# endorsement.
fairness_repeat_df <- study_df[
  (study_df$Condition == 'V') &
    (study_df$Motiv_To_Change),]

# Calculate sample size for analysis.
length(unique(fairness_repeat_df[, 'Participant']))

# Fit model predicting reflective endorsement.
fairness_repeat.glm <- glm(
  Repeat ~ Fairness_Reasoning,
  data = fairness_repeat_df,
  family = 'binomial'
)
summary(fairness_repeat.glm)

# Extract log odds and confidence intervals.
fairness_repeat_eff_df <- (
  as.data.frame(fairness_repeat.glm$coefficients)) %>%
  cbind(.,
        suppressMessages(confint(fairness_repeat.glm)),
        coef(summary(fairness_repeat.glm))[,4]) %>%
  setNames(., c('Log_Odds', 'CI_95_Lower', 'CI_95_Upper', 'P_Value'))

# Compute odds ratios.
fairness_repeat_odds_ratio_df <- fairness_repeat_eff_df %>%
  mutate(Log_Odds = exp(Log_Odds),
         CI_95_Lower = exp(CI_95_Lower),
         CI_95_Upper = exp(CI_95_Upper)) %>%
  rename(Odds_Ratio = Log_Odds)
fairness_repeat_odds_ratio_df

# Extract effect of fairness-based reasoning on reflective endorsement.
results_df['Fairness_Repeat',] <- fairness_repeat_eff_df['Fairness_Reasoning',]
...
# Save data.
write.csv(as.data.frame(adjusted_results_df), 'voi_ai_study_1_results.csv')
