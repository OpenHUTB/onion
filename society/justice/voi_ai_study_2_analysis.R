...
# Read study data.
raw_study_data <- read.csv('voi_ai_study_2_data.csv')
...
# Inspect cell counts. Sufficient for Breslow-Day test.
...
breslow_day_tab <- xtabs(
  cbind(Motiv_To_Maximize, Motiv_To_Prioritize) ~ Condition + Repeat,
  freq_tab_df)
breslow_day_tab

# Run Breslow-Day test.
breslow_day_results <- BreslowDayTest(breslow_day_tab, correct = T)
breslow_day_results

# Extract p-value from Breslow-Day test. The test does not produce an odds ratio
# or confidence intervals, but the p-value can still be included in our
# adjustment for multiple comparisons.
results_df['Breslow_Day',] <- c(NA, NA, NA, breslow_day_results$p.value)
...
# Save data.
write.csv(as.data.frame(adjusted_results_df), 'voi_ai_study_2_results.csv')