...
# Read study data.
raw_study_data <- read.csv('voi_ai_study_3_data.csv')
...
study_df <- raw_study_data %>%
  ...
  mutate(
    ...
    Saw_Correct_Harvest = as.logical(Saw_Correct_Harvest),
...
# Branch dataframe for analysis. Filter out participants who experienced a
# misordered harvest distribution (e.g., the third-positioned group member
# harvesting more than the first-positioned group member).
harvest_df <- study_df %>%
  filter(Saw_Correct_Harvest) %>%
  mutate(Motiv_To_Change = as.factor(Motiv_To_Change))
...
motiv_to_maximize_df <- harvest_df %>%
...
motiv_to_prioritize_df <- harvest_df %>%
...
# Save data.
write.csv(as.data.frame(adjusted_results_df), 'voi_ai_study_3_results.csv')