...
class HierarchicalAttentiveRecurrentTracker(Model):
    ...
    def __init__(self, inpt, bbox0, presence0, batch_size, glimpse_size,
        ...
        self.feature_extractor = feature_extractor  # Alex Net
    ...
    def _build(self):
        ...
        first_state = self.cell.zero_state(self.batch_size, tf.float32, self.bbox0, self.presence0, self.inpt[0],
                                           self.transform_init_features, self.transform_init_state)  # for output activation information

        # LSTM ?
        # time_major：决定了输出tensor的格式，
        # 如果为True, 张量的形状必须为 [max_time, batch_size,cell.output_size]。
        # 如果为False, tensor的形状必须为[batch_size, max_time, cell.output_size]
        ...
        if self.debug:
            (outputs, attention, presence, glimpse) = raw_outputs[:4]   # n*1*100(rnn output), n*1*4, n*1*1, n*1*9408
            shape = (-1, self.batch_size, 1) + tuple(self.glimpse_size)  # -1*1*1 * 56*56*3
            self.glimpse = tf.reshape(glimpse, shape, 'glimpse_shape')   # g_t: n*1*1*56*56*3
            tf.summary.histogram('rnn_outputs', outputs)
        else:
            (outputs, attention, presence) = raw_outputs[:3]

        if self.dfn_readout:  # apply Dynamic Filter Network
            self.obj_mask_logit = tf.reshape(raw_outputs[-3], (-1, self.batch_size, 1) + tuple(self.feature_shape))  # n*1*196 -> n*1*1*14*14
            self.obj_mask = tf.nn.sigmoid(self.obj_mask_logit)  # n*1*1*14*14 (0,1)
            obj_mask_features_flat = tf.reshape(raw_outputs[-2][1:], (-1, 10))  # dorsal stream output: s_t
        ...
        self.states_flat = tf.reshape(outputs[1:], (-1, self.rnn_units), 'flatten_states')  # ventral stream output: \upsilon_t
        if self.dfn_readout:
            self.states_flat = tf.concat(axis=1, values=(self.states_flat, obj_mask_features_flat))  # s_t(n*100) * \upsilon_t(n*10) -> s_t(n*110)

        # MLP: multi-layered perceptron: MLP(s_t * v_t) -> v_t(1*4)
        self.hidden_to_bbox = MLP(self.states_flat, self.rnn_units, 4, transfer=tf.nn.tanh, name='fc_h2bbox',
                             weight_init=self.cell._rec_init, bias_init=tf.constant_initializer())

        if self.debug:
            tf.summary.histogram('bbox_diff', self.hidden_to_bbox)

        attention = tf.reshape(attention, (-1, self.batch_size, 1, self.cell.att_size), 'shape_attention') # n*1*4 -> n*1*1*4
        self.attention = tf.concat(axis=0, values=(self.cell.att0[tf.newaxis], attention[:-1]))  # 1*1*1*4, n*1*1*4 -> n*1*1*4
        self.att_pred_bbox = self.cell.attention.attention_to_bbox(self.attention)
        self.att_pred_bbox_wo_bias = self.cell.attention.attention_to_bbox(self.attention - self.cell.att_bias)  # attention - attention bias
        self.att_region = self.cell.attention.attention_region(self.attention)  # attention region

        pred_bbox_delta = tf.reshape(self.hidden_to_bbox.output, (-1, self.batch_size, 1, 4), 'shape_pred_deltas')  # \delta b_t
    ...
    def loss(self, target_bbox, target_presence, scale=None, loss_type='mean_squared_error',
        ...
        if not hasattr(self, '_loss'):
            losses = AdaptiveLoss()  # Adaptive Loss Weights
            ...
            if loss_type.lower() == 'intersection_over_union':
                bbox_loss = _iou_loss(self.pred_bbox[t0:], target_bbox[t0:], float_presence[t0:])  # Tracking loss
            else:
