...
class AlexNetModel(FeatureExtractor):
    ...
    def _build(self):
        ...
        inpt = self.inpt
        if self.upsample:
            inpt = tf.image.resize_bilinear(inpt, (227, 227))  # AlexNet takes inputs of size 227*227 and downsizes them to 14*14 after conv3 layer
        o = alexnet(inpt, np.load(self.path, allow_pickle=True).item(), self.layer, maxpool=self.upsample)
        self.orig_output = o
