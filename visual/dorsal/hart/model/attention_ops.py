...
def extract_glimpse(inpt, attention_params, glimpse_size):
    ...
    # The matrix Fx, Fy contains one Gaussian per row;
    # the width and positions of Gaussians determine which parts of the image are extracted as the attention glimpse.
    ...
    for channel in tf.unstack(inpt, axis=rank - 1):
        g = tf.matmul(tf.matmul(Fy, channel, adjoint_a=True), Fx)  # g = A^y * x * A^x
        gs.append(g)
...
class Attention(object):
    ...
    def extract_glimpse(self, inpt, raw_att, return_all=False):
        ...
        # att_flat: attention parameters
        att_flat = self._to_attention(raw_att_flat)  # FixedStdAttention._to_attention()
        ...
        for a in tf.unstack(att, axis=1):
            glimpse.append(self._extract_glimpse(inpt, a))  # a: attention parameters
    ...
    def _extract_glimpse(self, inpt, att_flat):
        return extract_glimpse(inpt, att_flat, self.glimpse_size)  # call extract_glimpse() method outside all class
...
class FixedStdAttention(Attention):
    """Like RATM (Recurrent Attentive Tracking Model)
    but std for the gaussian mask depends directly and exclusively on the stride between gaussians.
    I used a small neural net to compute std to approximate bicubic
    interpolation and then fitted a 4th order surface to predictions of the neural net.
    centres, strides, (strides -> variances)
...
class AttentionCell(RNNCell):
    def __init__(self, feature_extractor, n_units, att_gain, glimpse_size,
        ...
        # Spatial Attention
        self.attention = attention_module(self.input_size, self.glimpse_size)  # RATM: Recurrent Attentive Tracking

        if not isinstance(zoneout_prob, (tuple, list)):
            zoneout_prob = (zoneout_prob, 0.)

        self.zoneout_prob = zoneout_prob  # dropout

        self.cell = self._make_cell(is_training)  # LSTM
    ...
    # init state (cell.zero_state是所有RNN单元状态的初始值设定项。)
    def zero_state(self, batch_size, dtype, bbox0=None, presence=None, img0=None,
        ...
        state = att0, presence
        zero_state = self.cell.zero_state(batch_size, dtype)  # Return zero-filled state tensor(s);  LSTM
        if img0 is not None:
        ...
        return state  # [shape_first_att(1*4), shape_first_presence(1*1), zero_state(1*100,1*100), rnn_outputs(1*100)
    ...
    def _zero_state(self, img, att, presence, state, transform_features, transform_state=False):
        ...
        with tf.variable_scope(self.__class__.__name__) as vs:
            self.features = self.extract_features(img, att)[1]  # get flatten x_t, v_t, flatten g_t
            # np.savetxt('result/activation/V1', np.squeeze(sess.run([model.cell.features], feed_dict)[0]))

            if transform_features:
                features_flat = tf.reshape(self.features, (-1, self.n_units))
                features_flat = AffineLayer(features_flat, self.n_units, name='init_feature_transform').output
                self.features = tf.reshape(features_flat, tf.shape(self.features))

            rnn_outputs, hidden_state = self._propagate(self.features, state)
    ...
    def _make_cell(self, is_training):
        ...
        raw_cell = IdentityLSTMCell if self.identity_init else LSTMBlockCell
    ...
    def extract_features(self, inpt, raw_att, apperance_vec=None, reuse=False):
        ...
        # Spatial Attention (x_t -> g_t), raw attribute flatten
        ...
        features = self.feature_extractor(glimpse_flat, reuse=reuse)  # AlexNet (V1 + ventral stream) output: nu^t

        # flatten features (vectorization)
        ...
        # The dorsal stream, implemented as a Dynamic Filter Network, is handling spatial relationships.
        if apperance_vec is not None:
            self.before_features = self.feature_extractor.orig_output
            # use appearance (predicted features, alpha_{t+1}) to dynamically compute convolutional filters (dfn_input).
            # the convolutional filters is learnable: (1)generate filters. (2) apply filter multiplication operation.
            self.dfn_inpt = DynamicFilterModel(self.before_features, apperance_vec, ksize=(1, 1), n_channels=5,
                                               name='pre_DFN')  # previous DFN
            self.dfn = DynamicFilterModel(self.dfn_inpt.features, apperance_vec, ksize=(3, 3), n_channels=5)
            dfn = self.dfn.features

            # Finally, a convolutional layer with a 1x1 kernel and sigmoid non-linearity is applied
            # to transform the output into a Bernoulli distribution s^t (obj_mask)
            obj_mask_logit = ConvLayer(dfn, (1, 1), 1, transfer=None, name='obj_mask').output
            obj_mask = tf.nn.sigmoid(obj_mask_logit)  # Dorsal Stream output: s^t
            features *= obj_mask

            self.flat_mask = tf.reshape(obj_mask, (self.batch_size * 1, -1))
        ...
        if apperance_vec is not None:
            output += (obj_mask_logit, self.flat_mask)
        return output

    def __call__(self, inpt, state, scope=None):
        raw_att, presence, hidden_state = state[:3]
        if self.predict_appearance:  # dorsal stream
        ...
        with tf.variable_scope(self.__class__.__name__):
            ...
            with tf.variable_scope(self.rnn_vs, initializer=self._rec_init, reuse=True):
                rnn_outputs, hidden_state = self._propagate(features, hidden_state)  # propagate through LSTM
            ...
            if self.predict_appearance:
                ...
                att_inpt = tf.concat(axis=1, values=(att_inpt, mask_features))  # s_t > \hat{s}_t

            # 2 fully connected, inner product layer
