...
clc;
addpath('utils');
proj_home = fileparts(fileparts(fileparts(fileparts( fileparts( mfilename('fullpath') ) ))));
...
X = load(fullfile(proj_home, 'data', 'data', 'neuro', 'TDdatabase'));
...
seg_dir = fullfile(proj_home, 'brain', 'visual', 'dorsal', 'data', 'seg', '*');
dirs = glob(seg_dir);
...
for dir = dirs'
    ...
    for sub = 1 : length(funcs)  % 暂时只处理一个，后面可以添加成多个受试
        ...
        for f = 1 : n_frame
            ...
            save(fullfile(intensity_dir, sprintf('%s_frame-%d_V1_intensity.txt', cur_sub, f)), 'V1_intensity', '-ascii');
            save(fullfile(intensity_dir, sprintf('%s_frame-%d_MT_intensity.txt', cur_sub, f)), 'MT_intensity', '-ascii');
            save(fullfile(intensity_dir, sprintf('%s_frame-%d_FEF_intensity.txt', cur_sub, f)), 'FEF_intensity', '-ascii');
        end
