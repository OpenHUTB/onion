#! /home/d/anaconda2/envs/hart/bin/python
# coding=UTF-8
########################################################################################
# 
...
from utils.env import ldconfig
ldconfig()  # or modify "/etc/ld.so.conf"
# set CUDA_10 lib path temporally (not taken effect) --> replace with 'loconfig'
# ld_path = os.getenv('LD_LIBRARY_PATH')
# cuda_lib_path = '/usr/local/cuda-10.0/lib64'
# if ld_path == None or not cuda_lib_path in ld_path:
#     if ld_path == None:
#         os.environ['LD_LIBRARY_PATH'] = cuda_lib_path
#     else:
#         os.environ['LD_LIBRARY_PATH'] += cuda_lib_path
#     try:
#         # raise:  OSError: [Errno 8] Exec format error -> chmod a+x scripts/train_hart_kitti.py;
#         # add "#! /home/d/anaconda2/envs/hart/bin/python" in the first line
#         os.execv(sys.argv[0], sys.argv)
#     except:
#         print('Exception: Failed to execute under modified environment')
#         exit(2)
...
parser.add_argument('--is_debug',
                    help='whether debug this program', type=bool, default=False)
...
learning_rate = .33e-5  # 3.33 x 10-6 ?
...
res_dir = 'result'
train_objs_path = 'result/train_objs.pkl'
...
train_store, train, test_store, test = get_data(args.img_dir, args.label_dir, .8, img_size[:2], schedule.n_timesteps,
                                                ...
                                                batch_size, n_threads=12, in_memory=True, sample_objects=-100,
                                                truncated_threshold=1., occluded_threshold=1, reverse=True, mirror=True)
# if args.is_debug:
#     with open(train_objs_path) as f:
#         train_store, train, test_store, test = pickle.load(f)  # Python 3: open(..., 'wb')
# else:
#     train_store, train, test_store, test = get_data(args.img_dir, args.label_dir, .8, img_size[:2], schedule.n_timesteps,
#                                                 schedule.max_timesteps,
#                                                 batch_size, n_threads=3, in_memory=True, sample_objects=-100,
#                                                 truncated_threshold=1., occluded_threshold=1, reverse=True, mirror=True)
#     with open(train_objs_path, 'w') as f:
#         pickle.dump([train_store, train, test_store, test], f)  # Python 3: open(..., 'rb')
