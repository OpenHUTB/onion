%% 在未训练的网络中，面部选择单元的不变特性 (Fig.S5,7) 

%% 对图像变化的不变性分析 (Fig.S5a-f)
...
for vtype = 1:TT
    ...
    switch vtype
        case 1
            disp('Position')
            % whos -file IMG_var_pos_210521.mat
            % 请求的 227x227x3x13000 (15.0GB)数组超过预设的最大数组大小(7.9GB)。这可能会导致 MATLAB 无响应。
            % load('IMG_var_pos_210521.mat'); IMG_var = single(repmat(permute(IMG_pos,[1 2 4 3]),[1 1 3])); clearvars IMG_pos
            % save(fullfile(data_dir, 'Stimulus', 'tmp_IMG_var_pos'), "IMG_var", '-v7.3');
            load('tmp_IMG_var_pos');
            load('IMG_var_pos_210521.mat', 'pos_idx', 'cls_idx');
        ...
        case 2
            ...
            % save(fullfile(data_dir, 'Stimulus', 'tmp_IMG_var_size'), "IMG_var", '-v7.3');
            load('tmp_IMG_var_size');
            load('IMG_var_size_210521.mat', 'size_idx', 'cls_idx');
        ...
        case 3
            ...
            % save(fullfile(data_dir, 'Stimulus', 'tmp_IMG_var_rot'), "IMG_var", '-v7.3');
            load('tmp_IMG_var_rot');
            load('IMG_var_rot_210521.mat', 'rot_idx', 'cls_idx');
    ...
    h = figure('units','normalized','outerposition',[0 0 1 1]); drawnow
    ...
    export_figure(h, fullfile(figs_dir, sprintf("fig_5_a-f_%d.pdf", TT)));
