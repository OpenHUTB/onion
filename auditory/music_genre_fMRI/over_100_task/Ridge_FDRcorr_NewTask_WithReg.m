function Ridge_FDRcorr_NewTask_WithReg(ID, Method)
%
%FDR correction for multiple comparison (Benjamini & Hochberg, 1995)
%Using novel tasks, with sensorimotor regressors
%
%Input:
...
load([SaveDir 'RidgeResult_WithReg_' Method '_' ID '_NewTask_All.mat'], 'Result');
ccs = Result.ccs;
...
V = MRIread(RefEPI);
V.vol = vol_perm;
MRIwrite(V,[SaveDir 'RidgeResult_WithReg_' Method '_FDRcorr_' ID '_NewTask_All.nii']);

Result.ccs_fdr = ccs;
Result.fdrthr = thrCCs;

save([SaveDir 'RidgeResult_' Method '_' ID '_NewTask_WithReg_All.mat'], 'Result', '-v7.3');
